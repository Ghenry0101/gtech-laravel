...
class BiteshipWebhookService
{
    ...
    private function isEmptyPayload(Request $request): bool
    {
    ...
    private function plainOk(): \Illuminate\Http\Response
    {
        return response('ok', 200)->header('Content-Type', 'text/plain');
    }

    private function shouldValidateSignature(Request $request): bool
    {
     ...
     */
    private function storeTrackingEvents(Shipment $shipment, array $payload): array
    {
    ...
    private function parseRecordedAt(array $event): Carbon
    {
    ...
    private function buildShipmentUpdates(
        Shipment $shipment,
        array $payload,
        ?string $latestStatus,
    ...
    private function extractWaybillId(array $payload): ?string
    {
        $candidates = [
            data_get($payload, 'waybill_id'),
    ...
    private function extractDriverField(array $payload, array $keys): ?string
    {
        $sources = [
            'driver',
            'courier.driver',
