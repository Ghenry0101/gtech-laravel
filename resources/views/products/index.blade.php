...
<body class="bg-slate-100 text-slate-900 antialiased font-aerospace">
  ...
  <main class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 bg-white p-6">
      <a href="{{ route('products.index') }}"
         class="group bg-white border rounded-md shadow-sm p-4 flex flex-col items-center gap-2 hover:shadow-md {{ $active ? '' : 'ring-2 ring-slate-900/10' }}">
      ...
      @foreach($categories as $cat)
        <a href="{{ route('products.index', ['categorySlug' => $cat['slug']]) }}"
           class="group bg-white border rounded-md shadow-sm p-4 flex flex-col items-center gap-2 hover:shadow-md {{ $active === $cat['slug'] ? 'ring-2 ring-slate-900/10' : '' }}">
