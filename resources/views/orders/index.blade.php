    ...
    <div class="py-10">
        <div class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-8">
            @if ($orders->isEmpty())
                <div class="rounded-md border border-dashed border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
                    <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-md bg-slate-100 text-slate-600">
                    ...
                    <a href="{{ route('home') }}" class="mt-6 inline-flex items-center rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
                        {{ __('Mulai Belanja') }}
                    </a>
                </div>
            @else
                    ...
                    @foreach ($orders as $order)
                        ...
                        <div class="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                @if ($status)
                                    <span class="inline-flex items-center rounded-md px-3 py-1 text-xs font-semibold {{ $status['badge_class'] }}">
                                        {{ $status['label'] }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <div class="flex flex-wrap gap-3">
                                    @if ($needsReview)
                                        <a href="{{ route('orders.show', $order).'#order-review-section' }}" class="inline-flex items-center rounded-sm bg-emerald-500 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600">
                                            {{ __('Tulis Ulasan') }}
                                        </a>
                                    @endif
                                    <a href="{{ route('orders.show', $order) }}" class="inline-flex items-center rounded-sm border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:border-slate-300 hover:text-slate-900">
                                        {{ __('Lihat Detail') }}
                                    </a>
                                </div>
                            </div>
